...
This launch sets a 3% tax on trades, and that 3% reaches the protocol in full. It is fixed at the moment the token is created and cannot be changed by anyone afterwards, including whoever holds the key. It lives on the launch venue rather than in these contracts, so it is checkable against the token itself.
...
Two things that look like powers are not. The wiring functions that point each contract at the others can each be called once and then revert forever, so after deployment they are spent. And whether bought tokens are burned or kept is fixed when the buyback contract is constructed, with no setter at all.

That list is meant to be exhaustive, and it is checkable: every function carrying the owner modifier is visible in the verified source, and there are 23 of them. If you find one this rulebook does not account for, the rulebook is wrong.
